...
struct entity_tpz {
    ...
    entity *ettptr;
    size_t vecpos;
    entity_tpz(entity_type type, entity *ettptr, size_t vecpos) : type(type), ettptr(ettptr), vecpos(vecpos) {}
};

struct entity : public sf::RectangleShape {
    b2BodyId b2bid;
    entity_tpz* tpz = new entity_tpz(entity_type::character, this, 0);
    entity(sf::Vector2f size, b2Vec2 init_pos, b2Rot init_rot, entity_type etype, b2WorldId b2wid, size_t vecpos, b2Vec2 init_speed = {0,0})
        ...
        if(tpz->type != entity_type::floor) shapeDef.enableContactEvents = true;
...
void proc_contact_events() {
    ...
    for(int i = 0; i < contact_events.beginCount; i++) {
        entity_tpz* etpz_x; entity_tpz* etpz_y;
...
void update(float dt, sf::RenderWindow*wndw) {
    ...
    if(ImGui::IsMouseClicked(ImGuiMouseButton_Left, true)) {
        ...
        sf::Vector2i mouse_pos = wndw->mapCoordsToPixel({(float)mouse_pos_pix.x,(float)mouse_pos_pix.y});
